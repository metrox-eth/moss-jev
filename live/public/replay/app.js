const constraints=await fetch('./constraints.json?v=4').then(r=>r.json());
let recordings;
try { const r=await fetch('./replay.json?v=4'); if(!r.ok)throw new Error('HTTP '+r.status); recordings=await r.json(); }
...
Object.assign(missions,Object.fromEntries(Object.entries(constraints).map(([k,c])=>[k,{title:c.title,instruction:c.instruction}])));
...
const forbidden=mesh(new T.CylinderGeometry(.18,.18,.008,64),new T.MeshBasicMaterial({color:0xff5368,transparent:true,opacity:.5}));forbidden.rotation.x=Math.PI/2;forbidden.position.set(.75,-.42,.01);forbidden.visible=false;
function constraintHUD(){const c=constraints[mission];forbidden.visible=!!c?.zone;
 $('constraint-state').textContent=c?`BATTERY ${c.battery}% · BIN ${c.full?'FULL':'AVAILABLE'}${c.zone?' · RED ZONE: NO COLLECTION':''}`:'Standard collection scenario';
...
function setMission(k){
 ...
 if(['home','battery','bin'].includes(k)){robot.position.set(.7,-.6,0);robot.rotation.z=Math.PI;}
 constraintHUD();log('Recorded scenario: '+k.toUpperCase());toast('NEW SCENARIO');running=true;buttons();
}
...
function obstacle(x,y){return (constraints[mission]?.zone&&Math.hypot(x-.75,y+.42)<.33)||obstacles.some(o=>Math.hypot(x-o.x,y-o.y)<o.r+.22)||Math.abs(x)>1.18||Math.abs(y)>1.02;}
...
async function decide(){if(busy||!running)return;busy=true;const rev=revision;status('RECORDED JEV DECISION','Replaying a real API response from the recorded run.');$('decision').textContent='Replaying…';const options=objects.filter(o=>!o.collected).map(o=>({id:o.id,kind:o.kind,reachable:!!approach(o),in_forbidden_zone:!!constraints[mission]?.zone&&o.id==='can_2',distance_m:+Math.hypot(o.x-robot.position.x,o.y-robot.position.y).toFixed(2)}));const state={instruction:missions[mission].instruction,robot:'MOSS tracked litter-collection rover in simulation',battery_percent:constraints[mission]?.battery??86,bin_full:!!constraints[mission]?.full||collected>=6,already_home:Math.hypot(robot.position.x-home.x,robot.position.y-home.y)<.06,objects:options};
...
try{const [g,m,health]=await Promise.all([new GLTFLoader().loadAsync('./assets/moss.glb?v=4'),fetch('./assets/motion.json?v=4').then(r=>r.json()),Promise.resolve({ready:true})]);model=g.scene;robot.add(model);motion=m;model.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;}if(o.name.startsWith('arm_'))armNodes[o.name.slice(4)]=o;});setArm(motion.travel);$('connection').textContent=health.ready?'RECORDED RUN':'API KEY REQUIRED';$('start').disabled=!health.ready;document.querySelectorAll('[data-mission]').forEach(b=>b.disabled=false);buttons();log('MOSS loaded. Recorded replay ready.');}catch(e){status('LOAD ERROR',e.message);$('connection').textContent='NOT READY';$('start').textContent='LOAD FAILED';console.error(e);}

constraintHUD();

// Camera controls: independent of mission reset, bounded and accessible.
const INITIAL_ZOOM=1.22;
function setZoom(value){camera.zoom=T.MathUtils.clamp(value,.65,3);camera.updateProjectionMatrix();$('zoom-value').textContent=Math.round(camera.zoom*100)+'%';}
setZoom(INITIAL_ZOOM);
$('zoom-in').onclick=()=>setZoom(camera.zoom*1.2);
$('zoom-out').onclick=()=>setZoom(camera.zoom/1.2);
$('zoom-reset').onclick=()=>setZoom(INITIAL_ZOOM);
const arena=document.querySelector('.arena');
arena.addEventListener('wheel',e=>{e.preventDefault();setZoom(camera.zoom*Math.exp(-T.MathUtils.clamp(e.deltaY,-120,120)*.0025));},{passive:false});
const fingers=new Map();let pinchDistance=0;
const pinch=()=>{const p=[...fingers.values()];return p.length===2?Math.hypot(p[0].x-p[1].x,p[0].y-p[1].y):0;};
renderer.domElement.style.touchAction='none';
renderer.domElement.addEventListener('pointerdown',e=>{fingers.set(e.pointerId,{x:e.clientX,y:e.clientY});renderer.domElement.setPointerCapture(e.pointerId);pinchDistance=pinch();});
renderer.domElement.addEventListener('pointermove',e=>{if(!fingers.has(e.pointerId))return;fingers.set(e.pointerId,{x:e.clientX,y:e.clientY});const d=pinch();if(d&&pinchDistance)setZoom(camera.zoom*d/pinchDistance);pinchDistance=d;});
for(const event of ['pointerup','pointercancel','lostpointercapture'])renderer.domElement.addEventListener(event,e=>{fingers.delete(e.pointerId);pinchDistance=pinch();});

const embedParams=new URLSearchParams(location.search);
if(embedParams.get('embed')==='1'){document.body.classList.add('embedded');const requested=embedParams.get('mission');if(missions[requested])setMission(requested);}
